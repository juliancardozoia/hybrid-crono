-- Los parametros de una categoria, en la ficha publica.
--
-- POR QUE FALTABAN
--
-- `division_movements` existe desde 20260901102300 con un proposito declarado:
-- que un atleta que navega el catalogo compare categorias y decida en cual
-- anotarse. "Elite Masculino: Thruster 43 kg, Clean and Jerk 60 kg" es lo que
-- decide una inscripcion.
--
-- La tabla se carga desde el panel y no llegaba a ninguna funcion `public_*`.
-- El unico peso que veia el publico salia de `division_movement_specs` —el peso
-- de un movimiento DENTRO de un WOD concreto— y solo despues de que el
-- organizador liberara esa prueba. O sea: meses tarde, y solo si llegaba a
-- cargar los WODs.
--
-- POR QUE NO VA DETRAS DE `released_at`
--
-- `released_at` protege el CONTENIDO de una prueba: que WOD sale y cuando se
-- revela lo decide el organizador, que los carga con semanas de anticipacion
-- para configurar la pantalla del juez.
--
-- El estandar declarado de la categoria es lo contrario: existe ANTES de que
-- haya una sola prueba cargada, y sigue existiendo aunque las pruebas cambien.
-- Esconderlo hasta que se libere un WOD invertiria el motivo por el que se
-- guarda.
--
-- `load_unit` viaja al JSON junto con los kilos. El kilo es el valor canonico
-- —es con lo que compara el motor— pero el atleta tiene que leer el numero del
-- reglamento: quien programo "95 lb" no reconoce "43,09 kg".

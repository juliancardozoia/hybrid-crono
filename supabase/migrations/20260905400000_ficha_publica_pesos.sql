-- La ficha publica dice el peso en la unidad del reglamento, y el cap de cada
-- categoria.
--
-- QUE FALTABA
--
-- 1. La UNIDAD del peso. `public_event_detail` devolvia solo los kilos, asi que
--    un WOD programado en libras se leia "43.09 kg" en la pantalla donde el
--    atleta espera "95 lb" — el numero redondo del reglamento. Las columnas
--    `load_unit` de `part_movements` y `division_movement_specs` existen desde
--    20260905300000 y no salian por ningun lado.
--
-- 2. El CAP POR CATEGORIA. `part_divisions.time_cap_ms` existe desde el dia uno
--    y no la leia nadie. Si Elite tiene 12 minutos y Scaled 15, el atleta lo
--    tiene que leer antes de elegir en cual se anota: es exactamente el tipo de
--    dato por el que se abre esta pantalla.
--
-- Lo que NO cambia: el contenido de una prueba sigue saliendo solo si
-- `released_at` ya paso, y los parametros de la categoria siguen saliendo
-- siempre (ver 20260905200000).
